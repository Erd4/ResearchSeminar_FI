...
merge_region <- function(df) {
  ...
  # Create Country_Region: "Country - Region" when Region exists, else just Country
  df$Country_Region <- ifelse(is.na(df$Region),
                              df$SP_COUNTRY_NAME,
                              paste(df$SP_COUNTRY_NAME, df$Region, sep = " - "))
...
#------------------------------------------------------------#
# Reorder: move Region + Country_Region right after SP_COUNTRY_NAME
#------------------------------------------------------------#
...
reorder_region <- function(df) {
  ...
  ctry_region_pos <- which(cols == "Country_Region")

  # Remove both from current positions, insert after SP_COUNTRY_NAME
  cols <- cols[-c(region_pos, ctry_region_pos)]
  country_pos <- which(cols == "SP_COUNTRY_NAME")
  cols <- append(cols, c("Region", "Country_Region"),
                 after = country_pos)
