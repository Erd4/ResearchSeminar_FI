...
#----Step 6a: Wild Cluster Bootstrap — Preferred Inference for Model 6----

# Wild cluster bootstrap addresses the small cluster problem (N=26 countries)
# Asymptotic clustered SEs are unreliable with fewer than 30 clusters
# Reference: Cameron & Miller (2015, Journal of Human Resources)
# Bootstrap method: Rademacher weights at cluster level
# Reference: Cameron, Gelbach & Miller (2008)
...
# Refit Model 6 on explicit dataset — needed for clean residual extraction
model6_check <- feols(
  Deposit_Ratio ~ age65_pct * FinStructure + 
    agg_replacement_ratio + Bismarckian + Beveridgean |
    Entity.ID + fiscal_year,
...
fitted_m6 <- fitted(model6_check)
resid_m6  <- residuals(model6_check)

# Cluster assignments
clusters        <- df_boot_base$SP_COUNTRY_NAME
unique_clusters <- unique(clusters)
n_clusters      <- length(unique_clusters)

# Store original coefficients — all country-level variables
coef_age65    <- coef(model6_check)["age65_pct"]
coef_finstr   <- coef(model6_check)["FinStructure"]
coef_interact <- coef(model6_check)["age65_pct:FinStructure"]
coef_arr      <- coef(model6_check)["agg_replacement_ratio"]

# Bootstrap distribution storage
set.seed(42)
B             <- 9999
boot_age65    <- numeric(B)
boot_finstr   <- numeric(B)
boot_interact <- numeric(B)
boot_arr      <- numeric(B)

# Progress tracking — prints every 1000 iterations
for(b in 1:B) {
  if(b %% 1000 == 0) cat(sprintf("Bootstrap iteration %d of %d...\n", b, B))
  
  # Draw Rademacher weights at cluster level
  weights        <- sample(c(-1, 1), n_clusters, replace = TRUE)
  names(weights) <- unique_clusters
  wild_weights   <- weights[clusters]
  
  # Perturb dependent variable
  df_boot_base$y_boot <- fitted_m6 + wild_weights * resid_m6
  
  # Refit model on perturbed data
  boot_fit <- feols(
    y_boot ~ age65_pct * FinStructure + agg_replacement_ratio |
      Entity.ID + fiscal_year,
    data    = df_boot_base,
    cluster = ~SP_COUNTRY_NAME,
    warn    = FALSE,
  ...
  boot_age65[b]    <- coef(boot_fit)["age65_pct"]
  boot_finstr[b]   <- coef(boot_fit)["FinStructure"]
  boot_interact[b] <- coef(boot_fit)["age65_pct:FinStructure"]
  boot_arr[b]      <- coef(boot_fit)["agg_replacement_ratio"]
}

# Compute bootstrap p-values (two-tailed)
p_boot_age65    <- mean(abs(boot_age65)    >= abs(coef_age65))
p_boot_finstr   <- mean(abs(boot_finstr)   >= abs(coef_finstr))
p_boot_interact <- mean(abs(boot_interact) >= abs(coef_interact))
p_boot_arr      <- mean(abs(boot_arr)      >= abs(coef_arr))

# Report results
cat("=== Wild Cluster Bootstrap Results (Model 6) ===\n")
cat(sprintf("Age65:           coef = %.4f | asymptotic p = %.4f | bootstrap p = %.4f\n",
            coef_age65,    0.1184, p_boot_age65))
cat(sprintf("FinStructure:    coef = %.4f | asymptotic p = %.4f | bootstrap p = %.4f\n",
            coef_finstr,   0.0551, p_boot_finstr))
cat(sprintf("Age65*FinStruct: coef = %.4f | asymptotic p = %.4f | bootstrap p = %.4f\n",
            coef_interact, 0.1171, p_boot_interact))
cat(sprintf("ARR:             coef = %.4f | asymptotic p = %.4f | bootstrap p = %.4f\n",
            coef_arr,      0.1268, p_boot_arr))
...
etable(model1, model2, model3, model4a, model4b, model6_preliminary,
       ...
       extralines = list(
         ...
         "_Fixed Effects"         = c("No", "No", "No", "No", "No", "Yes"),
         "_Boot p (Age65)"        = c("", "", "", "", "",
                                      round(p_boot_age65, 4)),
         "_Boot p (FinStructure)" = c("", "", "", "", "",
                                      round(p_boot_finstr, 4)),
         "_Boot p (Age65xFS)"     = c("", "", "", "", "",
                                      round(p_boot_interact, 4)),
         "_Boot p (ARR)"          = c("", "", "", "", "",
                                      round(p_boot_arr, 4))
       ),
